  ...
  fetch("/.netlify/functions/generate-route?to=" + url)
    ...
    .then(function (data) {
      document.querySelector(
        "#message"
      ).innerHTML = `<a href="${data.url}">${data.url}</a>`;
      return;
    });
}

/*
  if a shortcode URL brought us here, then the deployement of that redirect is still underway. So let's query the data store directly and send the user to the right
...
function redirectIfRequired() {
  ...
  if (path !== "/") {
    document.querySelector("#message").innerHTML =
      "The redirect rules for that short URL is still being created... sending you directly!";
    fetch("/.netlify/fuctions/get-route?code=" + path.replace("/", ""))
      .then(function (response) {
        return response.json();
      })
      .then(function (data) {
